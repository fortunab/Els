...
class LatentExperts(nn.Module):
    ...
    def forward(self, tokens, topk_idx):
        # Lightweight demonstration: average all experts.
...
class R_ULIx(nn.Module):
    """Barebone R-ULIx-style latent routing model.

    This is a structured, runnable skeleton extracted from the notebook idea.
    """
    def __init__(self, in_dim=128, shared_dim=256, num_experts=4, topk=2, num_classes=2):
        super().__init__()
        self.project = ProjectNormalize(in_dim, shared_dim, use_norm=True)
